...
namespace PlayerBots {
    ...
    enum class EBotWarmupChoice {
		...
		BuildStructures, // Build random structures
		RunAround, // Run around the lobby
		JumpAround, // Jump around the lobby
		PracticeBuilding, // Practice building in the lobby
        MAX // The default before bot picks choice
    };

    enum class EBotPersonality {
        Aggressive,   // Rushes enemy, minimal building, focuses on shooting
        Builder,      // Builds extensively, takes high ground, box fights
    ...
    public:
        bool CanBuild(PlayerBot* bot, EBotBuildingType BuildType) {
            ...
            if (BuildType == EBotBuildingType::Wall || BuildType == EBotBuildingType::Stair) {
                MaterialCost = 10;
            } else if (BuildType == EBotBuildingType::Floor || BuildType == EBotBuildingType::Cone) {
                MaterialCost = 10;
            }

            if (bot->WoodCount < MaterialCost) {
                return false;
            }
        ...
        void ExecuteBuildPattern(PlayerBot* bot, EBotBuildPattern Pattern) {
            ...
            switch (Pattern) {
                case EBotBuildPattern::DefensiveWall:
                    ExecuteDefensiveWall(bot);
                    break;
                case EBotBuildPattern::DefensiveBox:
                    ExecuteDefensiveBox(bot);
                    break;
                case EBotBuildPattern::Turtle:
                    ExecuteTurtle(bot);
                    break;
                case EBotBuildPattern::RampRush:
                    ExecuteRampRush(bot);
                    break;
                case EBotBuildPattern::Nineties:
                    ExecuteNineties(bot);
                    break;
                case EBotBuildPattern::HighGroundRetake:
                    ExecuteHighGroundRetake(bot);
                    break;
                default:
                    bot->bIsBuilding = false;
                    break;
        ...
        EBotBuildPattern GetBuildPatternForPersonality(PlayerBot* bot) {
            ...
            if (bot->Personality == EBotPersonality::Aggressive) {
                if (bot->bIsStressed && bot->Pawn->GetHealth() < 50) {
                    return EBotBuildPattern::DefensiveWall;
                }
                if (Dist < 800 && HeightDiff < -200) {
                    return EBotBuildPattern::RampRush;
                }
                return EBotBuildPattern::None;
            }
            else if (bot->Personality == EBotPersonality::Builder) {
                if (bot->bIsStressed) {
                    return EBotBuildPattern::Turtle;
                }
                if (HeightDiff > 200) {
                    return EBotBuildPattern::HighGroundRetake;
                }
                if (Dist < 1500 && UKismetMathLibrary::GetDefaultObj()->RandomBoolWithWeight(0.3f)) {
                    return EBotBuildPattern::Nineties;
                }
        ...
        void Tick(PlayerBot* bot) {
            ...
            if (bot->BotState == EBotState::Combat) {
                ...
                if (Pattern != EBotBuildPattern::None) {
                    ExecuteBuildPattern(bot, Pattern);
                }
            }
        }
    };
                ...
                if (!Nearest.IsZero()) {
                    ...
                    if (bot->PC->LineOfSightTo(bot->NearestPlayerPawn, BotLoc, true)) {
                        ...
                        if (!bot->bIsBuilding && bot->WoodCount > 50) {
                            ...
                            if (Pattern != EBotBuildPattern::None) {
                                BuildingService.ExecuteBuildPattern(bot, Pattern);
                            }
                        }
                    }
                    else {
    ...
    void TickBots() {
        ...
        for (PlayerBot* Bot : PlayerBotArray) {
            ...
            if (Bot->tick_counter == 0) {
                ...
                if (PersonalityRoll < 40) {
                    Bot->Personality = EBotPersonality::Balanced;
                } else if (PersonalityRoll < 75) {
                    Bot->Personality = EBotPersonality::Aggressive;
                } else {
                    Bot->Personality = EBotPersonality::Builder;
                }
                
                Bot->SkillLevel = UKismetMathLibrary::GetDefaultObj()->RandomIntegerInRange(3, 8);
                Bot->BuildCooldown = 0.15f - (Bot->SkillLevel * 0.01f);
