...
class BotsBTService_AIDropZone {
    public:
        void ChooseDropZone(PlayerBot* bot) {
            ...
            if (BuildingFoundations.Num() == 0) {
                Log("[DROP ZONE] No building foundations available!");
                return;
            }
            
            // If bot spawned on a roof, target large buildings (cities)
            if (bot->bHasRoofSpawned) {
                // Filter for large buildings only
                TArray<AActor*> LargeBuildings;
                for (AActor* Foundation : BuildingFoundations) {
                    if (!Foundation) continue;
                    FVector Origin, Extent;
                    Foundation->GetActorBounds(false, &Origin, &Extent);
                    float Footprint = Extent.X * Extent.Y * 4.0f;
                    if (Footprint >= 8000.0f) {
                        LargeBuildings.Add(Foundation);
                    }
                }
                
                if (LargeBuildings.Num() > 0) {
                    AActor* DropZone = LargeBuildings[UKismetMathLibrary::GetDefaultObj()->RandomIntegerInRange(0, LargeBuildings.Num() - 1)];
                    if (DropZone) {
                        FVector Origin, Extent;
                        DropZone->GetActorBounds(false, &Origin, &Extent);
                        float BuildingHeight = Extent.Z * 2.0f;
                        
                        bot->TargetDropZone = DropZone->K2_GetActorLocation();
                        bot->TargetDropZone.Z += (BuildingHeight > 300.0f ? BuildingHeight * 0.8f : 600.0f); // Target roof height
                        bot->TargetDropZone.X += UKismetMathLibrary::GetDefaultObj()->RandomFloatInRange(-300.f, 300.f);
                        bot->TargetDropZone.Y += UKismetMathLibrary::GetDefaultObj()->RandomFloatInRange(-300.f, 300.f);
                        
                        {std::stringstream ss; ss << "[DROP ZONE] Roof bot targeting large building roof at Z=" << bot->TargetDropZone.Z; Log(ss.str());}
                        return;
                    }
                }
            }
            
            // Normal drop zone selection
    ...
    public:
        void Tick(PlayerBot* bot) {
            // Null checks for crash prevention
            if (!bot || !bot->Pawn || !bot->PC || !bot->PlayerState) {
                Log("[CRASH FIX] Null bot components in AIDropZone::Tick!");
                return;
            }
            
            auto GameState = (AFortGameStateAthena*)UWorld::GetWorld()->GameState;
            if (!GameState) {
                Log("[CRASH FIX] GameState is null in AIDropZone::Tick!");
                return;
            }
            ...
            if (bot->BotState == EBotState::Bus) {
                ...
                if (!Bus) {
                    Log("[CRASH FIX] Aircraft is null in Bus state!");
                    return;
                }
                ...
                if (DistanceToDrop < bot->ClosestDistToDropZone) {
                    ...
                    if (Math->RandomBoolWithWeight(0.9f)) {
                        {std::stringstream ss; ss << "[BUS JUMP] Bot jumping at distance " << DistanceToDrop << " from drop zone"; Log(ss.str());}
                        bot->Pawn->BeginSkydiving(true);
                        bot->BotState = EBotState::Skydiving;
                        bot->bHasJumpedFromBus = true;
                        return;
                ...
                if (DistanceToDrop < 1500.f && Math->RandomBoolWithWeight(0.8f)) {
                    {std::stringstream ss; ss << "[BUS JUMP] Early jump at distance " << DistanceToDrop; Log(ss.str());}
                    bot->Pawn->BeginSkydiving(true);
                    bot->BotState = EBotState::Skydiving;
                    bot->bHasJumpedFromBus = true;
                }
    ...
    public:
        void Tick(PlayerBot* bot) {
            // Null checks for crash prevention
            if (!bot || !bot->Pawn || !bot->PC) {
                Log("[CRASH FIX] Null bot components in Combat::Tick!");
                return;
            }
            ...
            FVector BotLoc = bot->Pawn->K2_GetActorLocation();
            if (bot->NearestPlayerPawn && !bot->NearestPlayerPawn->IsDead()) {
    ...
    void TickBots() {
        ...
        if (!GameState) {
            Log("[CRASH FIX] GameState is null in TickBots!");
            return;
        }
            ...
            if (Bot->PlayerState->IsPlayerDead() || Bot->Pawn->IsDead()) {
                for (int i = 0; i < PlayerBotArray.size(); i++) {
                    if (PlayerBotArray[i]->Pawn == Bot->Pawn) {
                        if (GameMode->AliveBots.Num() == 0) {
                            Log("[CRASH FIX] AliveBots array is empty during death handling!");
                            break;
                        }
