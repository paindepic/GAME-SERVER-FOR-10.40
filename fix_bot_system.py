...
# Replace IsPlayerDead() with Health <= 0
content = re.sub(r'(\w+)\->IsPlayerDead\(\)', r'\1->Health <= 0', content)

# PART 2: Add new enums and structs
print("Adding new enums and structs...")

# Find ELootableType enum and add ERoofLootPhase and EBotDropType after it
...
if '        bool bHasRoofSpawned = false;' in content and new_fields not in content:
    content = content.replace(
...
# Find BotsBTService_Combat class end and insert new classes after it
if '    class BotsBTService_LobbyAdvanced {' not in content:
    content = content.replace(
        '''        }
    };
...
# PART 4: Add tick calls in warmup service
print("Adding tick calls for new services...")

# Find the warmup tick function and add calls to new services
warmup_tick_addition = '''
...
# Add tick calls after the warmup service
if 'BotsBTService_LobbyAdvanced LobbyAdvancedService;' not in content:
    # Find a good spot to add the tick calls - after BotsBTService_Warmup Tick function
    content = re.sub(
        r'(            default:\s*\n\s*break;\s*\n\s*\}\s*\n\s*);',
        r'\1\n' + warmup_tick_addition,
        content
...
print("  - GetHealth() -> Health")
print("  - GetShield() -> Shield")
print("  - GetMaxHealth() -> MaxHealth")
print("  - IsDead() -> Health <= 0")
print("  - IsPlayerDead() -> Health <= 0")
print("\nNew features added:")
print("  - ERoofLootPhase enum")
print("  - EBotDropType enum")
print("  - FBotTrajectory struct")
print("  - New warmup choices (FarmMaterials, PracticeNineties, PracticeEditing)")
print("  - New bot states (RoofBreaking, ChestLooting, HouseLooting)")
print("  - BotsBTService_LobbyAdvanced class")
print("  - BotsBTService_RoofLooting class")
print("  - New fields in PlayerBot struct for trajectory and roof looting")
